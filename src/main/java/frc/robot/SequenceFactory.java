...
public class SequenceFactory {
  ...
  public Command getDunkArmNoteTransferSequence() {
    return new ParallelDeadlineGroup(
            new SetDunkArmSlew(-25, rc.dunkArm), new SetShooterProfiled(0, rc.shooter))
        .andThen(
  ...
  public Command getSetRPMandShootCommand(double rpm, double angle) {
    return new InstantCommand()
        .andThen(
            new ParallelCommandGroup(getStopIntakingCommand(), getToShooterStateCommand(rpm, angle))
                .until(() -> getShooterOnTarget(angle, rpm))
                .withTimeout(2))
        .andThen(
            new ParallelCommandGroup(
                    new RunCommand(rc.passthrough::intake, rc.passthrough),
  ...
  // Furthermore we dont want to require the chassis when we dont want to
  // Note that there might be issues with the the angles at edge cases, likely will never use it for
  // that though
  public Command getTurnSetRPMandShootCommand(
      double targetBearing, double rpm, double shooterAngle) {
    return new InstantCommand()
        .andThen(
            new ParallelCommandGroup(
                    new RunCommand(rc.passthrough::stop, rc.passthrough),
                    new RunCommand(
                        () -> rc.chassis.driveToBearing(Math.toRadians(targetBearing)), rc.chassis),
                ...
                .until(
                    () ->
                        getShooterOnTarget(shooterAngle, rpm)
                            && Clamp.bounded(
                                rc.navx.getRotation2d().getDegrees(),
                                targetBearing - 10,
                                targetBearing + 10)
  ...
  public Command getStopShooterCommand() {
    return new ParallelCommandGroup(
            new SetShooterProfiled(0, rc.shooter), rc.shooterFlywheel.getShooterSetRPMCommand(0))
  ...
  public Command getTrapSequenceCommand(Command pathFollowingCommand, Measure<Angle> gyroAngle) {
    return new InstantCommand()
        .andThen(
            new RunCommand(() -> rc.chassis.driveToBearing(gyroAngle.in(Units.Radians)), rc.chassis)
                .withTimeout(3))
        // .until(
        //     ()->Clamp.bounded(rc.navx.getRotation2d().getDegrees(),
        // gyroAngle.in(Units.Degrees)-10, gyroAngle.in(Units.Degrees)+10) &&
        //     Clamp.bounded(rc.navx.getRate(), -5, 5)
        // )
        .andThen(
