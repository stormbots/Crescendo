...
public class ShooterSetOdometry extends Command {
  ...
  public ShooterSetOdometry(
      Shooter shooter, ShooterFlywheel flywheel, SwerveDrivePoseEstimator pe) {
  ...
  public void execute() {
    ...
    x =
        Units.Meters.of(
                Math.abs(pose.getX() - FieldPosition.getTargetList(TargetType.Speaker).getX()))
            .in(Units.Inches);
    y =
        Units.Meters.of(
                Math.abs(pose.getY() - FieldPosition.getTargetList(TargetType.Speaker).getY()))
            .in(Units.Inches);
