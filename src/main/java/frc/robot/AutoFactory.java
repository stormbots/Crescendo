...
public class AutoFactory {
  ...
  ProfiledPIDController thetaController =
      new ProfiledPIDController(
          1 / Math.PI / 5 * 2, 0, 0, new TrapezoidProfile.Constraints(Math.PI, Math.PI));
  ...
  boolean initAutoChooser() {
    ...
    autoChooser.addOption(
        "quasi forward", rc.chassis.sysIdQuasistatic(SysIdRoutine.Direction.kForward));
    autoChooser.addOption(
        "quasi backward", rc.chassis.sysIdQuasistatic(SysIdRoutine.Direction.kReverse));
    autoChooser.addOption(
        "dynamic forward", rc.chassis.sysIdDynamic(SysIdRoutine.Direction.kForward));
    autoChooser.addOption(
        "dynamic backward", rc.chassis.sysIdDynamic(SysIdRoutine.Direction.kReverse));

    // autoChooser.addOption("Two Meter Auto Path Planner", new
    // InstantCommand(()->rc.chassis.resetOdometry(new Pose2d(0,0, new
    // Rotation2d()))).andThen(pathPlannerFollowPathManual("twoMeterAuto")));
    // autoChooser.addOption("Rotate + Two Meter Auto Path Planner",
    // pathPlannerFollowPathManual("twoMeterRotation"));

    // autoChooser.addOption("auto Trap", new InstantCommand()
    //     .andThen(()->rc.chassis.setFieldCentricOffset(60))
    //     .andThen(()->rc.chassis.resetOdometry(new Pose2d(4.9, 4.1, new
    // Rotation2d(Units.Degrees.of(-60)))))
    //
    // .andThen(rc.sequenceFactory.getTrapSequenceCommand(pathPlannerFollowPathManual("ampTrap"),
    // Units.Degrees.of(-60)))
    // );
    ...
    //     .andThen(()->rc.chassis.setFieldCentricOffset(0))
    //     .andThen(()->rc.chassis.resetOdometry(new Pose2d(16.6-1.3, 5.65, new
    // Rotation2d(Units.Degrees.of(180)))))
    // );
  ...
  void initPathPlannerStuff() {
    ...
    Consumer<ChassisSpeeds> setSpeed =
        (chassisSpeeds) -> {
    ...
    NamedCommands.registerCommand(
        "intakeStop", rc.sequenceFactory.getStopIntakingCommand().withTimeout(5));

    NamedCommands.registerCommand(
        "subwooferShot", rc.sequenceFactory.getSetRPMandShootCommand(5500, 45));
    NamedCommands.registerCommand(
        "spinUpShotNoStop", rc.sequenceFactory.getToShooterStateCommand(7000, 32.5));
    NamedCommands.registerCommand(
        "stageShot", rc.sequenceFactory.getSetRPMandShootCommand(8000, 20));
    NamedCommands.registerCommand(
        "stageShotNoStop", rc.sequenceFactory.getToShooterStateCommand(8000, 20));
    NamedCommands.registerCommand(
        "stopFlywheel",
        new WaitCommand(0.1).andThen(new InstantCommand(() -> rc.shooterFlywheel.setRPM(0))));
    ...
    // NamedCommands.registerCommand("topSpinUpShotNoStop", new ShooterSetOdometry(rc.shooter,
    // rc.shooterFlywheel, new Pose2d(2.15, 6.5, new Rotation2d())).runForever());
    // NamedCommands.registerCommand("midSpinUpShotNoStop", new ShooterSetOdometry(rc.shooter,
    // rc.shooterFlywheel, new Pose2d(2.15, 6.5, new Rotation2d())).runForever());
    // NamedCommands.registerCommand("botSpinUpShotNoStop", new ShooterSetOdometry(rc.shooter,
    // rc.shooterFlywheel, new Pose2d(2.15, 6.5, new Rotation2d())).runForever());

    // NamedCommands.registerCommand("topNoteShotNoStop", new ShooterSetOdometry(rc.shooter,
    // rc.shooterFlywheel, new Pose2d(2.9, 7, new Rotation2d())).runForever());
    // NamedCommands.registerCommand("midNoteShotNoStop", new ShooterSetOdometry(rc.shooter,
    // rc.shooterFlywheel, new Pose2d(2.9, 5.55, new Rotation2d())).runForever());
    // NamedCommands.registerCommand("botNoteShotNoStop", new ShooterSetOdometry(rc.shooter,
    // rc.shooterFlywheel, new Pose2d(2.78, 4.9, new Rotation2d())).runForever());

    // NamedCommands.registerCommand("topShootPosShotNoStop", new ShooterSetOdometry(rc.shooter,
    // rc.shooterFlywheel, new Pose2d(4.3, 6.25, new Rotation2d())).runForever());
    // NamedCommands.registerCommand("midShootPosShotNoStop", new ShooterSetOdometry(rc.shooter,
    // rc.shooterFlywheel, new Pose2d(4.3, 4.85, new Rotation2d())).runForever());
    // NamedCommands.registerCommand("botShootPosShotNoStop", new ShooterSetOdometry(rc.shooter,
    // rc.shooterFlywheel, new Pose2d(3, 2.8, new Rotation2d())).runForever());

    NamedCommands.registerCommand(
        "topSpinUpShotNoStop", rc.sequenceFactory.getToShooterStateCommand(4000, 28.5 - 0.5));
    NamedCommands.registerCommand(
        "midSpinUpShotNoStop", rc.sequenceFactory.getToShooterStateCommand(1000, 32.5));
    NamedCommands.registerCommand(
        "botSpinUpShotNoStop", rc.sequenceFactory.getToShooterStateCommand(4000, 28));

    NamedCommands.registerCommand(
        "topNoteShotNoStop", rc.sequenceFactory.getToShooterStateCommand(5000, 20.7));
    NamedCommands.registerCommand(
        "midNoteShotNoStop", rc.sequenceFactory.getToShooterStateCommand(1000, 32.5));
    NamedCommands.registerCommand(
        "botNoteShotNoStop", rc.sequenceFactory.getToShooterStateCommand(5000, 20.5));

    NamedCommands.registerCommand(
        "topShootPosShotNoStop", rc.sequenceFactory.getToShooterStateCommand(5500, 14));
    NamedCommands.registerCommand(
        "topShootPosShotNoStop3", rc.sequenceFactory.getToShooterStateCommand(5500, 14));
    NamedCommands.registerCommand(
        "topShootPosShotNoStop4", rc.sequenceFactory.getToShooterStateCommand(5500, 14));
    NamedCommands.registerCommand(
        "midShootPosShotNoStop", rc.sequenceFactory.getToShooterStateCommand(6000, 15));
    NamedCommands.registerCommand(
        "botShootPosShotNoStop", rc.sequenceFactory.getToShooterStateCommand(6000, 14));
    NamedCommands.registerCommand(
        "visionShot",
        new ParallelCommandGroup(
            new VisionTurnToAprilTag(
                    () -> 0, () -> 0, () -> 0, rc.shooterVision, rc.chassis, rc.navx)
                .withTimeout(1),
            new ShooterSetVision(rc.shooter, rc.shooterVision, rc.shooterFlywheel)));

    NamedCommands.registerCommand(
        "rampUpVision", new RunCommand(() -> rc.shooterFlywheel.setRPM(4000)));
    ...
    PathPlannerLogging.setLogActivePathCallback(
        (list) -> pathPlannerField.getObject("path").setPoses(list));
  }
  ...
  SwerveControllerCommand generateSwerveControllerCommand(
      Pose2d start, List<Translation2d> midpoints, Pose2d end) {
    return generateSwerveControllerCommand(
        TrajectoryGenerator.generateTrajectory(start, midpoints, end, trajectoryConfig));
  }

  public Command makeBasicAuto(
      Pose2d start, List<Translation2d> midpoints, Pose2d end, double rpm, double angle) {
    ...
    return new InstantCommand()
        ...
        // .andThen(()->rc.chassis.resetOdometry(new Pose2d(start.getX(),start.getY(),
        // rc.navx.getRotation2d())))
        .andThen(rc.sequenceFactory.getSetRPMandShootCommand(rpm, angle))
        .andThen(
            new ParallelDeadlineGroup(
                generateSwerveControllerCommand(start, midpoints, end)
  ...
  public Command makeBasicAutoWithTurn(
      Pose2d start, List<Translation2d> midpoints, Pose2d end, double gyroAngleDegrees) {
    // Setting gyro offset + odometry
    // Preparing shooting
    // Shooting
    // Running path to note + intake
