...
public class Shooter extends SubsystemBase {
  ...
  public CANSparkMax shooterMotor =
      new CANSparkMax(Robot.isCompbot ? 14 : 13, MotorType.kBrushless);

  private SparkPIDController pidController = shooterMotor.getPIDController();
  private SparkAbsoluteEncoder shooterAbsEncoder =
      shooterMotor.getAbsoluteEncoder(SparkAbsoluteEncoder.Type.kDutyCycle);
  private double shooterSetPoint = 0.0;

  private double reverseSoftLimit = 2;
  private double forwardSoftLimit = 46;

  public static LUT lut =
      new LUT(
          new double[][] {
            {54, 42.5, 4000},
            {66, 38.3, 4000},
  ...
  public Shooter() {
    ...
    // pidController.setFeedbackDevice(shooterAbsEncoder);  //WARNING: This is potentially unsafe
    // due to controlling through the discontinuity at 0; do not use.
    pidController.setFeedbackDevice(
        shooterMotor.getEncoder()); // Make sure we revert to native encoder for PID
    ...
    shooterAbsEncoder.setVelocityConversionFactor(
        shooterAbsEncoder.getPositionConversionFactor()); // native unit is RPS
    ...
    shooterMotor
        ...
        .setVelocityConversionFactor(
            shooterMotor.getEncoder().getPositionConversionFactor()
                / 60.0); // Native unit is RPM, so convert to RPS
    syncEncoders();
  ...
  public boolean isOnTarget(double position) {
    ...
    // TODO figure out better tolerances that make sense
    return Clamp.bounded(
        shooterMotor.getEncoder().getPosition(), position - tolerance, position + tolerance);
  }
  ...
  public void setAngle(double degrees) {
    ...
    pidController.setReference(
        degrees, ControlType.kPosition, 0, getShooterFFPercent(), ArbFFUnits.kPercentOut);
  }
  ...
  public TrapezoidProfile.State getState() {
    // return new TrapezoidProfile.State(shooterAbsEncoder.getPosition(),
    // shooterAbsEncoder.getVelocity());
    return new TrapezoidProfile.State(
        shooterMotor.getEncoder().getPosition(), shooterMotor.getEncoder().getVelocity());
  }
