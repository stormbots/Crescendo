...
public class ShooterVision extends SubsystemBase {
  ...
  public void periodic() {
    ...
    // if (getVisibleTargetData().isPresent()) {SmartDashboard.putNumber("manualshoot/distance",
    // getVisibleTargetData().get().distance.in(Units.Inches));}
  ...
  public Optional<LimelightReadings> getVisibleTargetData() {
    ...
    double[] bp =
        camera.getEntry("targetpose_robotspace").getDoubleArray(new double[] {0, 0, 0, 0, 0, 0});
  ...
  public double getAngleToShooter() {
    ...
    var distanceToSpeaker = Math.sqrt(Math.pow(xToSpeaker, 2) + Math.pow(yToSpeaker, 2));
