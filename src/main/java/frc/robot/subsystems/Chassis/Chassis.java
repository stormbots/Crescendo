...
public class Chassis extends SubsystemBase {
  /** Creates a new Chassis. // * @param navx */
  ...
  /** Tuning notes: 1/pi is good 2/pi is too much, cause backlash */
  ...
  public void periodic() {
    ...
    swerveDrivePoseEstimator.updateWithTime(
        ...
        new SwerveModulePosition[] {
          ...
          frontRight.getPosition(), rearLeft.getPosition(),
  ...
  public void drive(
    ...
    if (rateLimit) {
      ...
      if (currentTranslationMag != 0.0) {
        ...
        directionSlewRate =
            500; // some high number that means the slew rate is effectively instantaneous
      }
  ...
  public SwerveModuleState[] getModuleStates() {
    return new SwerveModuleState[] {
      // frontLeft.getState(),
      frontRight.getState(), rearLeft.getState(),
      // rearRight.getState()
    };
  }
  ...
  /**
   * Apply an offset from initial navx zero to the intended "forward" direction for fieldcentric
   * controls. positive value rotates zero CW
   *
  ...
  public Command getFCDriveCommand(
      DoubleSupplier xPower, DoubleSupplier yPower, DoubleSupplier rotationPower) {
  ...
  public Command getDriveToBearingCommand(
      DoubleSupplier xPower, DoubleSupplier yPower, Supplier<Measure<Angle>> bearing) {
    return new RunCommand(
        () -> {
          driveToBearing(
              MathUtil.applyDeadband(xPower.getAsDouble(), OIConstants.kDriveDeadband),
  ...
  public void setIdleMode(IdleMode idleMode) {
    // for(MAXSwerveModule module : new MAXSwerveModule[]{fronLeft, frontRight, rearLeft,
    // rearRight}){
  ...
  public Measure<Distance> getDistanceFromStageCenter() {
    ...
    if (!isBlue) {
      stageCenterX = 16.542 - stageCenterX;
    }
    double stageCenterY = 4.1;

    double distanceInMeters =
        Math.hypot(currentPose.getX() - stageCenterX, currentPose.getY() - stageCenterY);
