...
public class ShooterFlywheel extends SubsystemBase {
  ...
  public void periodic() {
    ...
    SmartDashboard.putNumber(
        "/shooterFlywheel/topMotorCurrentRPM", topMotor.getEncoder().getVelocity());
    SmartDashboard.putNumber(
        "/shooterFlywheel/botMotorCurrentRPM", botMotor.getEncoder().getVelocity());
  ...
  public boolean isOnTarget() {
    ...
    // ADD RIGHT
    if (!Clamp.bounded(
        topMotor.getEncoder().getVelocity(), targetRPM - tolerance, targetRPM + tolerance))
      return false;
    if (!Clamp.bounded(
        botMotor.getEncoder().getVelocity(), targetRPM - tolerance, targetRPM + tolerance))
      return false;
    return true;
  }
